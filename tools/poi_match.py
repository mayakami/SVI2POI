...
import config
import json
from mycode.recall_type import read_json,filter_poi
import Levenshtein
import math
    ...
    for ref_poi in ref_pois:
        ...
        for poi_id, poi in poi_data.items():
            ...
            if ref_poi.get('match_type') == "osm":
                ...
                # 计算最终相似度分数
                final_score = 0.4 * text_similarity + 0.6 * distance_similarity
            else:
...
def evaluate_matching_results(match_path, recall_path):
    """评估匹配结果，计算召回率和平均距离误差"""
    ...
    for poi_id, poi in poi_data.items():
        if "match_POI" in poi:
            matched_pois += 1
            if poi["match_POI"]["type"] == "osm":
                osm_pois += 1
                if "distance" in poi["match_POI"]:
                    total_distance_error += poi["match_POI"]["distance"]
    precision_rate = matched_pois / total_pois if total_pois > 0 else 0
    avg_distance_error = total_distance_error / osm_pois if osm_pois > 0 else 0
    print(f"评估结果:")
    print(f"精确率: {precision_rate:.4f} ({matched_pois}/{total_pois})")
    ...
    plot_distance_boxplot(recall_path)
    # 返回评估指标
    return {
...
def find_best_match(poi, ref_pois, text_threshold=0.3, distance_threshold=100):
    ...
    for ref_poi in ref_pois:
        ...
        # 计算文本相似度
        ref_chinese, ref_english = split_chinese_english(ref_text)
        
        # 标准化处理后的名称
        ref_name = normalize_text(ref_text)
        ref_chinese = normalize_text(ref_chinese)
        ref_english = normalize_text(ref_english)
    ...
    for candidate in candidates:
        ...
        # 只对有坐标的参考POI计算距离
        if ref_poi.get("match_type") == "osm" and "lat" in ref_poi and "lon" in ref_poi:
...
def match_pois(poi_path, ref_path, output_path, mismatch_path):
    ...
    for poi_id, poi in poi_data.items():
        ...
        if best_match:
            # 创建match_POI字段
            if best_match.get("match_type") == "osm" and "lat" in best_match and "lon" in best_match:
                match_info = {
                    "type": best_match.get("match_type", "未知"),
...
def poi_match(args):
    ...
    # 执行匹配
    match_pois(poi_path, ref_path, match_path, mismatch_path)
    print(f"匹配完成！结果已保存到 {match_path}")
        # 执行匹配
    match_pois(poi_path, ref_path, match_path, mismatch_path)
    print(f"匹配完成！结果已保存到 {match_path}")
    ...
    # 执行反向匹配
    reverse_match_pois(poi_path, ref_path, recall_path)
    
    # 评估匹配结果
    evaluate_matching_results(match_path, recall_path)
