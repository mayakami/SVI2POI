...
import shutil
from mycode.text_similarity import calculate_similarity
from mycode.cal_poi import create_poi
from mycode.create_sign import pair_id_to_sign_ids, SIGNDatabase
import zhconv 

# 设置模块路径
# module_path = os.path.abspath(os.path.join(current_dir, 'tools', 'mycode'))
# if module_path not in sys.path:
#     sys.path.append(module_path)
# print(sys.path)
#from cal_poi import create_poi,filt_poi,read_poi
#from database import array_to_blob,blob_to_array
def is_point_in_bbox(point, bbox):
    #y,x = point
    x, y = point  # 修改为(x,y)顺序
...
def save_poi(args, POI_list, group_id = None):
    """
    POI_list = [{
    ...
    data[poi_id] = item
    """
    output_path = os.path.join(args.group_path, i, args.poi_json)
    data = {}
    poi_id =0
...
def create_signs(args,i):
    """
    创建并聚类招牌(signs)示例，包括数据加载、特征匹配和数据库存储
    
    该函数从指定目录加载检测结果和COLMAP重建数据，创建数据库连接，
    处理图像数据、3D点数据和标识牌数据，并建立不同图像中标识牌之间的对应关系。
    
    参数:
        args: 配置参数对象，包含以下字段:
            - dect_path: 检测结果JSON文件所在路径
            - group_path: 组数据根目录
            - colmap_path: COLMAP重建数据子目录
            - sign_path: 输出的标识牌数据库路径
            - error_matching_dir: 匹配错误记录目录
            - group_gps_csv: 包含图片GPS信息的CSV文件名
        
        i: 字符串，表示当前处理的 group id
    """

    # 获取路径和文件名
    #json_path = args.shopsign_json
    dect_path = args.dect_path
    group_dir = os.path.join(args.group_path, i)
    colmap_dir = os.path.join(group_dir, args.colmap_path)
    #output_dir = os.path.join(group_dir, 'signboard_entity_area1') 
    sign_path = os.path.join(group_dir, args.sign_path)
    error_matching_dir = args.error_matching_dir

    # 连接数据库
    images_path = os.path.join(colmap_dir, 'images.txt')
    matches_file = os.path.join(group_dir, 'matches.txt')
    ...
    if not relevant_images:
        #print("No relevant images found. Skipping sign creation.")
        return
